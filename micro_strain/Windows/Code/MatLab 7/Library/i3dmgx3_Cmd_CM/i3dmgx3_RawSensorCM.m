function [Packet,Error] = i3dmgx3_RawSensorCM(SerialLink,SampleRate)
...
if Error == 0
    ...
    if Error == 0
        [Packet,Record,Error] = i3dmgx3_ReceiveDataContinuous(SerialLink,'CMD_RAW_ACCELEROMETER',SampleRate); %Read data from device
        if Error ~= 0
            Packet = [];
        end
    else
