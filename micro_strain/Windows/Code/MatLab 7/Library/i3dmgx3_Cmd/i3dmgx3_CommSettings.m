function [Packet,Error] = i3dmgx3_CommSettings(SerialLink,PortSelector,FunctionSelector,BaudRate,PortConfig)
...
if Error == 0
    ...
    if Error == 0
        ...
        fwrite(SerialLink,0)
        [Packet,Error] = i3dmgx3_ReceiveData(SerialLink,'CMD_COMM_SETTINGS'); %Read data from device
        if Error ~= 0
            Packet = [];
        end
    else
