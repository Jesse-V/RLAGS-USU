...
/*                                                                            */
/* All the routines for decoding raw bayer data are contained in this module. */
/* This code is a cut-down version of bayer.c from the libdc1394 project by   */
...
typedef enum {
  ...
  DC1394_FUNCTION_NOT_SUPPORTED,  /*  warnings or info are negative numbers   */
...
dc1394error_t dc1394_bayer_NearestNeighbor_uint16 (
					     const uint16_t *restrict bayer, uint16_t *restrict rgb,
					     int sx, int sy, int tile, int bits);
dc1394error_t dc1394_bayer_Bilinear_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict rgb,
						 int sx, int sy, int tile, int bits);
dc1394error_t dc1394_bayer_HQLinear_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict rgb,
						 int sx, int sy, int tile, int bits);
dc1394error_t dc1394_bayer_EdgeSense_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict rgb,
						 int sx, int sy, int tile, int bits);
dc1394error_t dc1394_bayer_Downsample_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict rgb,
						 int sx, int sy, int tile, int bits);
dc1394error_t dc1394_bayer_Simple_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict rgb,
						 int sx, int sy, int tile, int bits);	
dc1394error_t dc1394_bayer_VNG_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict dst,
						 int sx, int sy,dc1394color_filter_t pattern, int bits);
static void cam_to_cielab (uint16_t cam[3], float lab[3]);
dc1394error_t dc1394_bayer_AHD_uint16 (
						 const uint16_t *restrict bayer, uint16_t *restrict dst,
						 int sx, int sy,dc1394color_filter_t pattern, int bits);
int debayer_get_tile (int pattern, short h_offset, short v_offset);
 ...
 * raw-Bayer pattern images. Most of the algos presented here *
 * come from  http://www-ise.stanford.edu/~tingchen/ and have *
 * been converted from Matlab to C and extended to all        *
 * elementary patterns.                                       *
 **************************************************************/
...
dc1394error_t dc1394_bayer_EdgeSense_uint16(
						 ...
						 int sx, int sy, int tile, int bits)
{
	/* Coriander's Bayer decoding, removed due to patent concerns */
...
int debayer_get_tile (int pattern, short h_offset, short v_offset)
{
	/* Given the starting pattern at the first pixel on the chip and the
	 * h and v offset into the chip of the actual image data, return the
	 * bayer tile at the start of the actual image data.
	 *
	 * The bayer tile is assumed to be one of the following:
	 *
	 * R G    G R    G B    B G
	 * G B    B G    R G    G R
	 *
	 * The possible input values for 'pattern' must be one of the following:
	 *
	 * 0 (R G)    1 (G R)    3 (G B)    4 (B G)
	 * 
	 * Note that the first pixel on the chip is assumed to be in the lower left
	 * corner (as for Kodak chips).  The h and v offsets must be zero for the 
	 * first pixel on the chip.
	 */
	
	const int TILE[] = {DC1394_COLOR_FILTER_RGGB, 
					    DC1394_COLOR_FILTER_GRBG, 
	...
	return TILE[pattern + h_offset%2 + 3 * (v_offset%2)];
}
